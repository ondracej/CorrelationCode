function [FR_meta, AnovaStats] = analysis_calcFR_binSpikes_Motifs_CON(BIN_size, OnOff_params, spks, raw_data_info, allDirPaths, stim_selected_by_user, doplot)
...
if isempty(OnOff_params.BOUT_stim_start) || isempty(OnOff_params.BOUT_stim_stop)
    ...
    for m = 1 : num_channels
        ...
        if chanSpksDuringStim > 0
            ...
            for p = 1 : unique_stims
                %i = p;
                chan_SpkTimesFullStims = [];
                chan_SpksCntsFullStims = [];
                chan_StimDurations_samp = [];
                chan_StimDurations_s = [];
                chan_FiringRatesFullStims = [];
                
                for s = 1 : reps_stims(p)
                    
                    %% Define stimulus onset offset
                    start_stim =  OnOff_params.stim_start{p}(s);
                    stop_stim =  OnOff_params.stim_stop{p}(s);
                    ...
                    chan_SpkTimesFullStims{s} = these_spks_on_chan;
                    chan_SpksCntsFullStims(s) = this_spike_count;
                    chan_StimDurations_samp(s) = length_this_stim;
                    chan_StimDurations_s(s) = length_this_stim/scanrate_labview;
                    
                    chan_FiringRatesFullStims(s) = this_spike_count /  chan_StimDurations_s(s);
                ...
                chan_SpkTimesFullStims_uniqueMotifs{p} = chan_SpkTimesFullStims;
                chan_SpksCntsFullStims_uniqueMotifs{p} = chan_SpksCntsFullStims;
                chan_StimDurations_samp_uniqueMotifs{p} = chan_StimDurations_samp;
                chan_StimDurations_s_uniqueMotifs{p} = chan_StimDurations_s;
                chan_FiringRatesFullStims_uniqueMotifs{p} = chan_FiringRatesFullStims;
            ...
            allFiringRates_Stim_MOTIF{1, chan} = chan_FiringRatesFullStims_uniqueMotifs;
            allDurations_s_Stim_MOTIF{1, chan} = chan_StimDurations_s_uniqueMotifs;
            allDurations_samps_Stim_MOTIF{1, chan} = chan_StimDurations_samp_uniqueMotifs;
            allSpkCnts_Stim_MOTIF{1, chan} = chan_SpksCntsFullStims_uniqueMotifs;
            allSpkTimes_Stim_MOTIF{1, chan} = chan_SpkTimesFullStims_uniqueMotifs;
            ...
            for p = 1 : OnOff_params.bout_unique_stims
                ...
                for s = 1 : OnOff_params.bout_reps
                    ...
                    %% Define stimulus onset offset
                    start_stim =  OnOff_params.BOUT_stim_start(i);
                    stop_stim =  OnOff_params.BOUT_stim_stop(i);
            ...
            for p = 1 : unique_stims
                ...
                for s = 1 : reps_stims(p)
                    length_this_stim = allDurations_samps_Stim_MOTIF{1, chan}{1,p}(s);
    ...
    for q = 1 : num_channels
        ...
        if isempty(allFiringRates_Stim{this_chan})
            ...
            %% Newly added
            allChans_FR_vec_Stims_MOTIF{1, this_chan} = [];
            
            allChans_FR_vec_Stims_reps{1, this_chan} = [];
            allChans_FR_vec_Spont_reps{1, this_chan} = [];
            allChans_FR_vec_Stims_reps_MOTIF{1, this_chan} = [];
