function [] = collectDatafromCriteriaFile_ForSpont(CriteriaSelection)

dbstop if error

hardcodeCriteria = 1;
stimOrSpont = 0; % 1 stim, 0, spont
...
if stimOrSpont == 0
    ...
    for q = 1: n_meta
        dashes2 = find(names_metaSorted{q} == dash);
        dots2 = find(names_metaSorted{q} == dot);
        ...
        str2 = names_metaSorted{q}(dashes2+1:dots2-1);
...
for s = 1: numel(criteriaMetaNames)
    ...
    if ~isempty(chansForMetaFile)
        ...
        %% Make sure that we are loading the right file
        
        dash = '-';
        dot = '.';
        
        dashes1 = find(criteriaMetaNames{s} == dash);
        dots1 = find(criteriaMetaNames{s} == dot);
        ...
        str1 = criteriaMetaNames{s}(dashes1(end)+1:dots1-1);
       
        fileMatchInd = find(cell2mat(cellfun(@(x) strcmpi(x, str1), tagNames, 'uniformoutput', 0)));
        ...
        if fileMatch
            this_metafile = [metaDir names_metaSorted{fileMatchInd}];
    ...
    if stimOrSpont
        ...
        data_name = [ saveStr 'Spont--Pairs-' num2str(number_of_pairs) '_Neurons-' num2str(number_of_neurons)]; %
    end
    ...
    function [metaDir, names_metaSorted, n_meta]  = findSpontandStimMetaDirs(mainDataDir, runForStimFiles)
        ...
        if runForStimFiles
            ...
            spontMetaDir = [spontDir 'PairwiseOutput/meta/NewMeta/'];
