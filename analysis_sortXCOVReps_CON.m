function[COV_sorted_C_matrix, COH_sorted_C_matrix] = analysis_sortXCOVReps_CON(C_Matrix, OnOff_params, FR_meta, stim_selected_by_user)
    ...
    for m = 1: num_channels
        ...
        for j = 1 : unique_stims
            ...
            if OnOff_params.stim_analysis_type ==5 && stim_selected_by_user ==7
                TL_reps_this_stim = cell(1, allreps(j));
                TS_reps_this_stim = cell(1, allreps(j));
                reps = allreps(j);
            else
            ...
            runs = 1;
             ...
             COV_TL_reps_this_stim = [];
            COH_TL_reps_this_stim  = [];
            
            
               COV_TS_reps_this_stim  = [];
                COH_TS_reps_this_stim  = [];
                
                COV_TS_np1_reps_this_stim  = [];
                COH_TS_np1_reps_this_stim  = [];
...
%             end
        ...
        if sum(sum(cellfun(@(x) isempty(x), COV_all_stims_TL))) == numel(COV_all_stims_TL)
           ...
           % bla = cellfun(@(x) reshape(x, 1, numel(x)), COV_all_stims_TL, 'uniformoutput', 0)
