function [FR_meta, AnovaStats] = analysis_calcFR_binSpikes(BIN_size, OnOff_params, spks, raw_data_info, allDirPaths, stim_selected_by_user, doplot)
...
if OnOff_params.stim_analysis_type == 5
    ...
    for m = 1 : num_channels
        ...
        if chanSpksDuringStim > 0
            ...
            chan_SpkTimes_Spont = cell(unique_stims, reps_stims);
            chan_SpksCnts_Spont = nan(unique_stims, reps_stims);
            chan_SpontDurations_samp = nan(unique_stims, reps_stims);
            chan_SpontDurations_s = nan(unique_stims, reps_stims);
            chan_FiringRatesSpont = nan(unique_stims, reps_stims);
            ...
            for p = 1 : unique_stims
                ...
                for s = 1 : reps_stims
                    ...
                    start_stim = OnOff_params.stim_start(i) - spontInterval_samps ;
                    stop_stim = OnOff_params.stim_start(i);
            ...
            %% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %% Start for loop binning the stimulus evoked spikes
            %% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            for p = 1 : unique_stims
                ...
                for s = 1 : reps_stims
                    length_this_stim = chan_StimDurations_samp(p, s);
            ...
            if stim_selected_by_user ~=7 && doplot == 1 || OnOff_params.stim_analysis_type == 4 && doplot == 1;
                this_title = {['Binned Rasterplot, Chan-' num2str(chan) ': ' OnOff_params.song_type ', f mean: ' num2str(roundn(nanmean(f), -2))]};
                ...
                this_save_name = [save_dir raw_data_info.recording_session '-' OnOff_params.song_type '-Chan-' num2str(chan) '.png'];
